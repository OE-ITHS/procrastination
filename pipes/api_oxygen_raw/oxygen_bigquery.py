...
def generate_df_and_config(oxygen_data: dict, schema: bigquery.SchemaField) -> tuple[pd.DataFrame, bigquery.LoadJobConfig]:
    '''
    Generates a pandas DataFrame and a bigquery LoadJobConfig object for loading data to bigquery.
    
    :param weather_data:
        oxygen_data is expected to be a dictionary generated from response.json() in the api_oxygen.fetch_current_data() function.
        Directly feed the returned dict object from api_oxygen.fetch_current_data() into this function.
    :returns: :object:
        Returns a oxygen_df that contains the weather data as well as a job_config object for the load_to_bigquery() function.
    :rtype: tuple(pandas.DataFrame, bigquery.LoadJobConfig)
    '''
    # Transforms oxygen_data from json to json string for "schemaless" storage.
    # (Entire json stored as one single column and row value)
    oxygen_data_string = json.dumps(oxygen_data)
    ...
    return oxygen_df, job_config

def load_to_bigquery(oxygen_df: pd.DataFrame, job_config:bigquery.LoadJobConfig) -> bool:
    '''
    Loads data to a bigquery table with a "schemaless" table, e.i. stored as one value.

    :param oxygen_df: 
        oxygen_data is expected to be a pandas DataFrame generated from generate_df().
    ...
    table_id = 'acquired-sound-433108-c6.oxygen_data.oxygen_raw'
